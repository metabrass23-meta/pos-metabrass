                  ...
                  Consumer<SalesProvider>(
                    builder: (context, salesProvider, child) {
                      ...
                      return PremiumDropdownField<String>(
                        label:
                            l10n.selectSaleRequired ?? "Select Sale (Required)",
                        ...
                        return Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildAmountRow(
                                'Total Amount:',
                                'PKR ${selectedSale.grandTotal.toStringAsFixed(2)}',
                              ),
                              _buildAmountRow(
                                'Paid Amount:',
                                'PKR ${selectedSale.amountPaid.toStringAsFixed(2)}',
                              ),
                              _buildAmountRow(
                                'Remaining Amount:',
                                'PKR ${selectedSale.remainingAmount.toStringAsFixed(2)}',
                                isRemaining: true,
                              ),
                              _buildAmountRow(
                                'Payment Status:',
                                selectedSale.isFullyPaid
                                    ? 'Fully Paid'
                                    : 'Partially Paid',
                              ),
                            ],
  ...
  Future<void> _createInvoice() async {
    ...
    try {
      ...
      if (success && mounted) {
        ...
        final createdInvoice = invoiceProvider.invoices.firstWhere(
          (inv) => inv.saleId == _selectedSaleId,
          orElse: () => throw Exception('Created invoice not found'),
        );

        debugPrint(
          '🔍 [CreateInvoiceDialog] Created invoice: ${createdInvoice.invoiceNumber}',
        );

        // Get the sale data for PDF generation
        final salesProvider = context.read<SalesProvider>();
        final sale = salesProvider.sales.firstWhere(
          (s) => s.id == _selectedSaleId,
          orElse: () => throw Exception('Sale not found'),
        );

        debugPrint(
          '🔍 [CreateInvoiceDialog] Found sale: ${sale.invoiceNumber}',
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Invoice Created and PDF Generated Successfully"),
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: 13,
              color: isRemaining ? Colors.red.shade700 : Colors.black87,
